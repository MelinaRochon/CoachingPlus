...
final class TranscriptManagerTests: XCTestCase {
    ...
    func testGetTranscript() async throws {
        ...
        XCTAssertNotNil(transcript, "Transcripts should exist")
    ...
    func testGetAllTranscripts() async throws {
        ...
        XCTAssertNotNil(transcripts, "Transcripts should exist")
    ...
    func testAddNewTranscript() async throws {
        ...
        let transcript = try await manager.getTranscript(teamId: teamId, gameId: gameId, transcriptId: transcriptId!)
        XCTAssertNotNil(transcript, "Transcript should exist")
    ...
    func testDeleteAllTranscripts() async throws {
        ...
        let transcripts = try await manager.getAllTranscripts(teamId: "team1", gameId: gameId)
        XCTAssertTrue(transcripts?.isEmpty ?? false, "All transcripts should not exist")
    }
    ...
    func testUpdateTranscript() async throws {
        ...
        try await manager.updateTranscript(teamDocId: teamDocId, gameId: gameId, transcriptId: transcriptId, transcript: transcript)
        let updatedTranscript = try await manager.getTranscript(teamId: "team1", gameId: gameId, transcriptId: transcriptId)!
        XCTAssertNotNil(updatedTranscript, "Transcript should exist")
